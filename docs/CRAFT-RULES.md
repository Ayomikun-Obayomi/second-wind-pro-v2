...
- **M-06.** **Roster carousel:** step nav at all breakpoints — prev/next advances one card, start-aligned snap, smooth native scroll (~400ms settle), **unchanged card sizes** (300px desktop, ~full-width mobile). Homepage carousel is a featured partner set (5–7 cards); full roster lives on Meet the Athletes. **Contain scroll (trimSnaps):** no trailing end-spacer; natural `maxScroll` leaves the last card flush with the viewport end (Embla `containScroll: 'trimSnaps'` / Nike-style). Dots = unique lead-card stops (`cards − visible + 1`), not one snap per card into empty space. **Leadership carousel:** step nav; mobile uses horizontal swipe (`overflow-x: auto`, scroll-snap); desktop uses button nav with `overflow-x: hidden`.
